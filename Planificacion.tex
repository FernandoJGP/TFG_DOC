...
Además, como también figura en la tabla, se ha decidido establecer un periodo denominado "`\textbf{Red de seguridad}"' a modo de medida de contingencia para paliar cualquier retraso que surja durante la ejecución del proyecto. Esto es una medida recomendable en todos los proyectos pero más ante el que nos encontramos, ya que es el primer proyecto de este tipo / tecnología. En concreto tendremos un colchón de seguridad de dos semanas y media.
...
{Cálculo de horas por rol en iteraciones}
	...
	\end{coolTable}
	\caption{Tabla del cálculo de horas por rol en iteraciones}
\end{table*}
...
Uno de los datos más importantes que tenemos que analizar es la desviación en cuanto a tiempo total que se ha producido al término del proyecto. En este caso, han sido necesarias \textbf{31,5 horas} más de las planificadas para finalizar, lo que supone un 10\% sobre el total de horas planeadas en un principio (que fueron un total de 310). Esto se debe en gran parte a la pequeña re-planificación que se ha efectuado durante la última iteración para añadir "`funcionalidad"' extra, ya que recordemos que se decidió alargar una semana esta iteración para mejorar los escenarios de juego que componen el software, aprovechando la red de seguridad de la que disponíamos. Además, teniendo en cuenta que es el primer proyecto con esta tecnología y de este tipo no es una diferencia demasiado significativa o que no fuese esperable en un principio.
